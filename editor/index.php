...
define('PATTERN_FILES', '/^[A-Za-z0-9-_.\/]*\.(txt|php|conf|htm|html|js|css|tpl|md|xml|json)$/i'); // empty means no pattern
...
session_name('onixsat');
session_start();

if (empty(PASSWORD) === false && (isset($_SESSION['onixsat_admin'], $_SESSION['onixsat_password']) === false || $_SESSION['onixsat_admin'] !== true || $_SESSION['onixsat_password'] != PASSWORD)) {
    if (isset($_POST['onixsat_password']) && empty($_POST['onixsat_password']) === false) {
        $password_hash = hash('sha512', $_POST['onixsat_password']);

        if ($password_hash === PASSWORD) {
            session_regenerate_id(true);

            $_SESSION['onixsat_admin'] = true;
            $_SESSION['onixsat_password'] = $password_hash;
    ...
    die('<title>OnixSat</title><form method="post"><div style="text-align:center"><h1><a href="http://github.com/onixsat/onixsat" target="_blank" title="PHP file editor" style="color:#444;text-decoration:none" tabindex="3">OnixSat</a></h1>' . (isset($error) ? '<p style="color:#dd0000">' . $error . '</p>' : null) . '<input id="onixsat_password" name="onixsat_password" type="password" value="" placeholder="Password&hellip;" tabindex="1"><br><br><input type="submit" value="Login" tabindex="2"></div></form><script type="text/javascript">document.getElementById("onixsat_password").focus();</script>');
}

if (isset($_GET['logout'])) {
    if ($_GET['logout'] == $_SESSION['onixsat_token']) {
        unset($_SESSION['onixsat_admin']);
...
if (isset($_GET['path'])) {
    ...
    $session_token = $_SESSION['onixsat_token'] ?? null;
...
$_SESSION['onixsat_token'] = bin2hex(random_bytes(32));
...
<body>
    <?php if (PASSWORD == hash('sha512', 'admin')) : ?>
        <div class="heading-alert alert alert-warning"><i class="fa fa-info-circle"></i><span class="ms-2">Você está usando o OnixSat com a senha padrão. Por favor, clique <a href="javascript:void(0);" class="change-password">aqui</a> para alterar a senha após a instalação.</span></div>
    <?php endif; ?>
    ...
    <div class="container-fluid">
        ...
        <div class="row p-3">
            <div class="col-md-3">
                <h1><a href="http://github.com/onixsat/onixsat" target="_blank" title="OnixSat <?= VERSION ?>">OnixSat</a></h1>
            </div>
            <div class="col-md-9">
                ...
                <div class="float-end">
                    ...
                    <?php if (in_array('changepassword', $permissions)) { ?><a href="javascript:void(0);" class="change-password btn btn-sm btn-primary"><i class="fas fa-key"></i></a> &nbsp; <?php } ?><a href="<?= $_SERVER['SCRIPT_NAME'] ?>?logout=<?= $_SESSION['onixsat_token'] ?>" class="btn btn-sm btn-danger"><i class="fas fa-sign-out-alt"></i></a>
                </div>
            </div>
        </div>
    ...
    <form method="post">
        ...
        <input name="token" type="hidden" value="<?= $_SESSION['onixsat_token'] ?>">
